...
(function ($) {
    ...
    function SlickGrid(container, data, columns, options) {
        ...
        var rowHeights = {
            ...
            offset:[],
            sum:0,
            findLastRow:function (totalHeight) {
                for (var i = 0; i < this.offset.length; i++) {
                    if (this.offset[i] >= totalHeight) {
                        return Math.max(i - 1, 0) + 0.5;
                    }
                }
            ...
            updateOffset:function () {
                ...
                for (var i = 0; i < this.height.length; i++) {
                    this.offset[i] = this.sum;
                    this.sum = this.sum + this.height[i];
                }
            },
            initialize:function (dataLength) {
                if (this.height.length < dataLength) {
                    for (var i = this.height.length; i < dataLength; i++) {
                        if (i >= 1) {
                            this.offset[i] = this.offset[i - 1] + this.height[i - 1];
                        } else {
                            this.offset[0] = 0;
                        }
                        this.height[i] = options.rowHeight;
                    }
                } else {
                    this.height.splice(dataLength, this.height.length - dataLength);
                    this.offset.splice(dataLength, this.offset.length - dataLength);
                }
            ...
            updateSum:function () {
                this.sum = 0;
                for (var i = 0; i < this.height.length; i++) {
                    this.sum += this.height[i];
                }
            },

            initializeFrom:function (changedRow) {
                if (changedRow < this.height.length) {
                    for (var i = changedRow; i < this.height.length; i++) {
                        if (i >= 1) {
                            this.offset[i] = this.offset[i - 1] + this.height[i - 1];
                        } else {
                            this.offset[0] = 0;
        ...
        function appendRowHtml(stringArray, row, range) {
             ...
             */
            if (!!options.enableDynamicRowHeight) {
                stringArray.push("<div class='ui-widget-content " + rowCss + "' style='top:" + (rowHeights.offset[row] - offset) + "px'>");
            } else {
        ...
        function updateRow(row) {
             ...
             * edited to support dynamic row height
             */
            var maxCellHeight = options.rowHeight;
            ...
            for (var columnIdx in cacheEntry.cellNodesByColumnIdx) {
                 ...
                 */
                if (!!options.enableDynamicRowHeight) {
                    var cellHeight = node.height;
                    if (cellHeight && cellHeight > maxCellHeight) {
             ...
             */
            if (!!options.enableDynamicRowHeight) {
                rowHeights.height[row] = maxCellHeight;
            }
        ...
        function resizeCanvas() {
            ...
            if (options.autoHeight) {
                ...
                if (!!options.enableDynamicRowHeight) {
                    ...
                    viewportH = options.rowHeight * (options.enableAddRow ? 1 : 0) + rowHeights.sum;
                } else {
                    viewportH = options.rowHeight * (getDataLength() + (options.enableAddRow ? 1 : 0));
                }
                numVisibleRows = getDataLength() + (options.enableAddRow ? 1 : 0);
//                viewportH = options.rowHeight * (getDataLength() + (options.enableAddRow ? 1 : 0));
            } else {
...
//            numVisibleRows = Math.ceil(viewportH / options.rowHeight);
        ...
        function updateRowCount() {
             ...
             */
            if (!!options.enableDynamicRowHeight) {
                rowHeights.initialize(numberOfRows);
            }
             ...
             */
            if (!!options.enableDynamicRowHeight) {
                th = Math.max(rowHeights.sum, viewportH - scrollbarDimensions.height);
            } else {
...
//            th = Math.max(options.rowHeight * numberOfRows, viewportH - scrollbarDimensions.height);
        ...
        function getVisibleRange(viewportTop, viewportLeft) {
            ...
            if (!!options.enableDynamicRowHeight) {
                return {
                    top:Math.floor(rowHeights.findLastRow(viewportTop + offset)),
                    bottom:Math.ceil(rowHeights.findLastRow(viewportTop + offset + viewportH)),
                    leftPx:viewportLeft,
        ...
        function updateRowPositions() {
            ...
            if (!!options.enableDynamicRowHeight) {
                for (var row in rowsCache) {
                    rowsCache[row].rowNode.style.top = (rowHeights.sum - offset) + "px";
                }
            } else {
...
//            }
        ...
        function commitEditAndSetFocus() {
            ...
            if (getEditorLock().commitCurrentEdit()) {
                ...
                if (!!options.enableDynamicRowHeight) {
                    invalidateAllRows();
                    rowHeights.initializeFrom(activeRow);
                    render();
                }
        ...
        function scrollRowIntoView(row, doPaging) {
             ...
             */
            if (!!options.enableDynamicRowHeight) {
                if (row < rowHeights.offset.length - 1) {
                    rowAtTop = rowHeights.offset[row + 1];
                } else {
                    rowAtTop = rowHeights.sum;
                }
            } else {
...
//          var rowAtTop = row * options.rowHeight;
            var rowAtBottom = rowAtTop + options.rowHeight - viewportH + (viewportHasHScroll ? scrollbarDimensions.height : 0);
        ...
        function scrollRowToTop(row) {
            ...
            if (!!options.enableDynamicRowHeight) {
                if (row < rowHeights.offset.length - 1) {
                    scrollTo(rowHeights.offset[row + 1]);
                } else {
                    scrollTo(rowHeights.sum);
                }
            } else {
